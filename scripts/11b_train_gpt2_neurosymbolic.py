#!/usr/bin/env python3
"""
11b_train_gpt2_neurosymbolic.py  —  v2.6.final

NEURO-SYMBOLIC GPT-2 FINE-TUNING

Companion to scripts/10b_prepare_gpt2_neurosymbolic.py. Trains
jcblaise/gpt2-tagalog on the multi-token-conditioned corpus produced
by script 10b.

WHY THIS REPLACES SCRIPT 11
---------------------------
The v2.5 training script (scripts/11_train_gpt2MINERVA.py) registered
only six special tokens: <|label=real|>, <|label=fake|>, and four DE-GNN
graph tokens. v2.6.final's neuro-symbolic conditioning needs eighteen
special tokens (label + graph + qlat + ensem + tier), so we register
the full set from scripts/10b_prepare_gpt2_neurosymbolic.py.

PRESERVATION GUARANTEE (the user's "retain pretrained status" ask)
------------------------------------------------------------------
The base GPT-2-Tagalog model has:
  - Token embedding matrix of shape (V_base, 768) where V_base ~32000
  - Transformer layers + LM head with millions of parameters

After this script:
  - The token embedding matrix is RESIZED to (V_base + 18, 768) where
    18 = len(ALL_SPECIAL_TOKENS).
  - The first V_base rows are byte-identical to the pretrained checkpoint.
  - Only the 18 NEW rows (~14 KB) are randomly initialized and trained.
  - All transformer layer weights start identical to the pretrained
    checkpoint and are gently fine-tuned at low LR (5e-5, 3 epochs).
  - The base linguistic competence is preserved; the model learns to
    USE the new control tokens to steer its existing generation
    capability — it doesn't have to relearn Tagalog.

This achieves the user's "retain pretrained status, utilize what we
need" objective via the most-natural-for-the-paper mechanism (control
tokens) — not via a generic technique like LoRA that has no link to
the M.I.N.E.R.V.A. neuro-symbolic claim.
...
- Keskar et al. (2019). CTRL: A Conditional Transformer Language Model
  for Controllable Generation. arXiv:1909.05858. — basis for control
  tokens as conditioning mechanism.
- Wolf et al. (2020). Transformers: State-of-the-Art Natural Language
  Processing. EMNLP Demos. — HuggingFace Transformers stack.
- Cruz, Tan, & Cheng (2020). Localization of Fake News Detection via
  Multitask Transfer Learning. LREC 2020. — JCBlaise dataset citation;
  also the source of the base gpt2-tagalog checkpoint we adapt.
"""
